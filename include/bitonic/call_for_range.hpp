...
public:
    static void call(Functor &&f) {
        ...
        CallForRangeImpl<Index + 1, Size - 1, Functor>::call(std::forward<Functor>(f));
    }
};
...
void call_for_range(Functor &&f) {
    meta_detail::CallForRangeImpl<0, Size, Functor>::call(std::forward<Functor>(f));
}

//! Call a generic functor (like a generic lambda) for the integers [Begin,End).
template<size_t Begin, size_t End, typename Functor>
void call_for_range(Functor &&f) {
    meta_detail::CallForRangeImpl<Begin, End - Begin, Functor>::call(std::forward<Functor>(f));
}
